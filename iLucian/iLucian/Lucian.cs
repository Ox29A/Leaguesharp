...
namespace iLucian
{
    internal class Lucian
    {
        ...
        private static void OnEnemyGapcloser(CActiveCGapcloser gapcloser)
        {
            ...
            if (!gapcloser.Sender.IsEnemy || !(gapcloser.End.Distance(ObjectManager.Player.ServerPosition) < 300))
                return;
        ...
        // Detuks ofc
        private static Vector3 GetExtendedPrediction(Obj_AI_Base target, int delay)
        {
        ...
        private void OnLaneclear()
        {
                    ...
                    MinionTeam.NotAlly))
            {
                ...
                {
                    if (collision.Count < Variables.Menu.Item("com.ilucian.laneclear.qMinions").GetValue<Slider>().Value)
                        continue;
